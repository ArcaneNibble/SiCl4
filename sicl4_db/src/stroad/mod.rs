            ...
            if shard.nents > 0 {
                ...
                for (bucket_i, bucket) in buckets.iter().enumerate() {
                    ...
                    while let Some(ll_ent) = ll.next {
                        ...
                        // dbg!(&ll_ent.key);
                        println!("Stroad shard 0x{shard_i:X} bucket 0x{bucket_i:X} still wanting lock {:?} from work item {:?}", ll_ent.key(), &ll_ent.work_item_link as *const _);
                        any_bad_found = true;

                        ll = &ll_ent.link;
                        if ll as *const _ == &bucket.wants_lock as *const _ {
                            break;
                        }
                    }
                }
