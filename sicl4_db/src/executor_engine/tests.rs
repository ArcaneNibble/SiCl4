...
fn yolo_wtf_ordered() {
    ...
    let manager = NetlistManager::new();
    // fixme wtf is this???
    let fake_workqueue = work_queue::Queue::new(NTHREADS, 128);
    let real_workqueue = ordered_commit_queue::OrderedCommitQueue::new();
    let start_mem = memory_stats().unwrap();

    {
        let heap_thread_shard = manager.heap.new_thread();
        let mut init_thread_view = manager.access_single_threaded(&fake_workqueue);
        let mut rng = rand_xorshift::XorShiftRng::seed_from_u64(0);
        ...
        let start_queue = Instant::now();
        {
            for _ in 0..N_INITIAL_WORK {
                ...
                let (new, _gen) = heap_thread_shard.allocate::<WorkItem>();
                let work_item = unsafe { WorkItem::init(new.as_mut_ptr(), lut.into()) };
                real_workqueue.create_new_work(ItemWithPriority {
                    item: &*work_item,
                    prio: 0,
                });
                // init_thread_view.add_work(lut.into(), 0);
